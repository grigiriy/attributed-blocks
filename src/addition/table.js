	...
	return props => {
		...
		return (
			<Fragment>
				<BlockEdit {...props} />
				<InspectorControls>
					<PanelBody title={__("Table options")} initialOpen={true}>
						<ToggleControl>
							label="Has thead" checked={thead == true}
							onChange=
							{() => {
								setAttributes({
									thead: !thead
...
wp.hooks.addFilter(
	...
	"blocks/create-thead-attributes",
	createHeaderField
);
wp.hooks.addFilter(
	"editor.BlockEdit",
	"blocks/add-thead-field",
	addHeaderField
);
wp.hooks.addFilter(
	"blocks.getSaveContent.extraProps",
	"blocks/get-extended-table",
	getExtendedTable
);
