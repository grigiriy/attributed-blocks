...
const addSchemaFields = createHigherOrderComponent(BlockEdit => {
	return props => {
		...
		const { itemprop, itemtype, anchor } = props.attributes;
...
const getExtendedElements = (props, blockType, attributes) => {
	const { itemprop, itemtype, anchor } = attributes;
...
wp.hooks.addFilter(
	...
	"blocks/create-schema-attributes",
	createSchemaAttributes
);
wp.hooks.addFilter(
	"editor.BlockEdit",
	"blocks/add-schema-fields",
	addSchemaFields
);
wp.hooks.addFilter(
	"blocks.getSaveContent.extraProps",
	"blocks/get-extended-elements",
	getExtendedElements
);
wp.hooks.addFilter(
	"blocks.getSaveElement",
	"blocks/attributes-create",
	addAttributes
);
